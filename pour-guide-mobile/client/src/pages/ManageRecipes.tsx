...
import { Archive, ArrowLeft, Check, ChevronRight, ImagePlus, Loader2, Minus, Plus, Save, Search, Settings2, Upload } from "lucide-react";
...
function ManageRecipesContent() {
  ...
  const visibleRecipes = useMemo(() => recipesQuery.data?.filter(recipe => [recipe.name, recipe.koreanName, recipe.base, recipe.category].filter(Boolean).join(" ").toLowerCase().includes(listQuery.trim().toLowerCase())) ?? [], [listQuery, recipesQuery.data]);
  ...
  const initializeMutation = trpc.recipes.initializeDefaults.useMutation();
  const isSaving = saveMutation.isPending || uploadMutation.isPending || initializeMutation.isPending;
  ...
  const initializeDefaults = async () => {
    try {
      ...
      await utils.recipes.list.invalidate();
      if (result.recipes[0]) setSelectedId(result.recipes[0].id);
      toast.success(result.created ? "기본 칵테일을 관리 목록에 추가했습니다." : "이미 저장된 관리 메뉴를 불러왔습니다.", { description: "왼쪽 목록에서 메뉴를 눌러 바로 수정하세요." });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "기본 메뉴를 불러오지 못했습니다.");
    }
  };
  ...
  return <div className="mx-auto max-w-6xl pb-12"><header className="flex flex-wrap items-end justify-between gap-4 border-b border-black/15 pb-5"><div><p className="font-mono text-[10px] font-bold tracking-[0.16em] text-[#bf3f32]">MANAGEMENT / RECIPES</p><h1 className="mt-2 font-serif text-4xl tracking-[-0.04em]">메뉴와 레시피 관리</h1><p className="mt-2 text-sm text-[#706b61]">왼쪽 목록에서 기존 칵테일을 선택하면 즉시 수정할 수 있습니다.</p></div><button onClick={() => setLocation("/")} className="flex items-center gap-2 text-sm font-bold text-[#5d584f]"><ArrowLeft size={16} /> 바텐더 화면</button></header>
    <div className="mt-7 grid gap-7 lg:grid-cols-[285px_minmax(0,1fr)]"><aside className="border border-black/15 bg-[#f3eee5]"><div className="flex items-center justify-between border-b border-black/15 p-4"><span className="font-mono text-[10px] font-bold tracking-[0.14em]">EXISTING RECIPES</span><button onClick={() => setSelectedId("new")} className="grid h-8 w-8 place-items-center rounded-full bg-[#bf3f32] text-white" aria-label="새 메뉴 추가"><Plus size={16} /></button></div><div className="border-b border-black/10 p-3"><label className="relative block"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#817b71]" /><input value={listQuery} onChange={event => setListQuery(event.target.value)} className="h-9 w-full border border-black/15 bg-white pl-9 pr-3 text-xs" placeholder="기존 메뉴 찾기" /></label></div><div className="max-h-[60dvh] overflow-y-auto p-2">{recipesQuery.isLoading ? <div className="p-5 text-sm text-[#777268]"><Loader2 className="inline animate-spin" size={15} /> 불러오는 중</div> : <>{(recipesQuery.data?.length ?? 0) === 0 ? <div className="m-1 border border-dashed border-black/20 bg-white p-4"><p className="font-serif text-lg">수정할 메뉴를 불러오세요.</p><p className="mt-2 text-xs leading-5 text-[#706b61]">바텐더 화면의 기본 4개 칵테일을 관리 가능한 목록으로 추가합니다.</p><button onClick={initializeDefaults} disabled={isSaving} className="mt-4 w-full bg-[#171714] px-3 py-2.5 text-xs font-bold text-white disabled:opacity-50">{initializeMutation.isPending ? "불러오는 중..." : "기본 칵테일 관리하기"}</button></div> : <><button onClick={() => setSelectedId("new")} className={`w-full border-l-2 px-3 py-3 text-left text-sm font-bold ${selectedId === "new" ? "border-[#bf3f32] bg-white" : "border-transparent"}`}>+ 새 메뉴 만들기</button>{visibleRecipes.map(recipe => <button key={recipe.id} onClick={() => setSelectedId(recipe.id)} className={`w-full border-t border-black/10 px-3 py-3 text-left ${selectedId === recipe.id ? "border-l-2 border-l-[#bf3f32] bg-white" : "hover:bg-white/60"}`}><span className="block font-serif text-lg">{recipe.name}</span><span className="mt-1 block text-[11px] text-[#777268]">{recipe.koreanName || recipe.base} · {recipe.status === "active" ? "판매 중" : "중단"}</span></button>)}{visibleRecipes.length === 0 && <p className="p-4 text-xs text-[#706b61]">검색 결과가 없습니다.</p>}</>}</>}</div></aside>
